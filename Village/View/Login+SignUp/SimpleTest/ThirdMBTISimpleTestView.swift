...
class ThirdMBTISimpleTestView: BaseView {
    
    private let titleLabel: UILabel = {
        let label = UILabel()
        label.text = "👉 문제가 생겼을 때  "
    ...
    let firstSelectButton: UIButton = {
        ...
        button.setTitle("이해가 안되는데 공감은 간다", for: .normal)
    ...
    let secondSelectButton: UIButton = {
        let button = UIButton(type: .system)
        button.setTitle("이해가 되야 공감을 하든 말든 하지", for: .normal)
        button.setTitleColor(.grey300, for: .normal)
        button.backgroundColor = .grey100
        button.titleLabel?.font = .suitFont(size: 15, weight: .regular)
        button.contentHorizontalAlignment = .leading
    ...
    func selected(mbtiDecisions: MBTIDecisions) {
        
        switch mbtiDecisions {
        case .F:
